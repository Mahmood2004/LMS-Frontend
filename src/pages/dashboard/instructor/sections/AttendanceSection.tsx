...
import InstructorAttendanceServices, {
  AttendanceStatus,
  StudentRoster,
  SubmitAttendancePayload,
} from "@/services/instructor/attendanceService";
...
const AttendanceSection = ({ selectedCourseId }: AttendanceSectionProps) => {
  ...
  useEffect(() => {
    if (selectedCourseId) {
      setSelectedCourse(selectedCourseId.toString());
    }
  }, [selectedCourseId]);
    ...
    const fetchStudent = async () => {
      try {
        ...
        const fetchedStudents =
          await InstructorAttendanceServices.getCourseRoster(
            String(selectedCourse),
          );
        setStudents(fetchedStudents.roster);
        const defaultAttendance: Record<string, AttendanceStatus> = {};
        fetchedStudents.roster.forEach((s) => {
          defaultAttendance[s.student_id] = "absent";
        });
  ...
  const handleSaveAndExport = async () => {
    ...
    try {
      ...
      if (!currentSessionId) {
        const res = await InstructorAttendanceServices.createSession({
      ...
      const records: SubmitAttendancePayload["records"] = students.map((s) => ({
        student_id: s.student_id,
        status: attendance[s.student_id] ?? "absent",
      }));
      await InstructorAttendanceServices.submitAttendance(currentSessionId, {
        records,
      });
      ...
      toast({
        ...
        description: `Recorded for ${courses.find((c) => c.id === selectedCourse)?.title}.`,
        duration: 2000,
      });
    } catch (err) {
      console.error(err);
    ...
    <>
          ...
          students.map((student, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                <Initials name={student.student_name ?? "Student"} />
                <div>
      ...
      <div className="mt-4 flex justify-end">
        <Button
          ...
          disabled={!students.length || savingAttendance}
        >
